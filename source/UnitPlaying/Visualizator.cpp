...
QString Visualizator::buildInferiorArguments()
{
	...
	// Notice: a "_v" is append to output filenames in order to avoid overwriting the same files
	// generated by TestCaseActors when the player solution was run in batch against all test cases.
	// The "_v" suffix stands for the test case being visualized
	const QString& input = playerSolution->buildTestCaseFilepath(testCaseNumber, "input");
	const QString& output_ps = playerSolution->buildTestCaseFilepath(testCaseNumber, "output_ps_v");
	const QString& error_ps = playerSolution->buildTestCaseFilepath(testCaseNumber, "error_ps_v");
...
bool Visualizator::stepForward()
{
	...
	if (   debuggerCall->sendGdbCommand("-data-evaluate-expression bn_tell_stdin()",  visStandardInput ) == GDB_ERROR
		|| debuggerCall->sendGdbCommand("-data-evaluate-expression bn_tell_stdout()", visStandardOutput) == GDB_ERROR
