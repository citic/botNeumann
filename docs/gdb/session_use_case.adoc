...
. Extract function names with Universal ctags (Exuberant ctags does not support C++11)
. [line-through]#For each file in player solution that does not begin with bn_: send the command#
. For each function definition in `tags.txt`, set a breakpoint
. For each breakpoint response, create a GdbBreakpoint object with role FunctionBody
